...
function isMarketOpen() {
  ...
  return (h * 60 + m) >= 9 * 60 + 30 && (h * 60 + m) < 16 * 60;
}
...
function BiasChip({ bias }) {
  ...
  return <span style={{ fontSize: 9, letterSpacing: "0.08em", color, background: color + "18", border: `1px solid ${color}44`, borderRadius: 3, padding: "2px 6px", textTransform: "uppercase", fontFamily: "'IBM Plex Mono', monospace" }}>{bias || "—"}</span>;
}
...
export default function TradingBuddy() {
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: data.reply ? "assistant" : "error", content: data.reply || data.error || "No response", timestamp: replyTs }]);
    } catch (err) {
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 120px)", background: "#06060b", borderRadius: 8, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "#0a0a10", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button onClick={() => setMessages([])} style={{ fontSize: 9, fontFamily: "'IBM Plex Mono', monospace", background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#475569", borderRadius: 3, padding: "3px 8px", cursor: "pointer" }}>CLEAR</button>
        </div>
      </div>
      ...
      <div style={{ padding: "10px 14px", borderTop: "1px solid rgba(255,255,255,0.06)", background: "#0a0a10", flexShrink: 0, display: "flex", gap: 8, alignItems: "flex-end" }}>
        <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} disabled={loading} placeholder="Ask your trading buddy..." rows={1}
          style={{ flex: 1, background: "#0d1117", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "9px 12px", color: "#e2e8f0", fontSize: 12, fontFamily: "'DM Sans', sans-serif", resize: "none", outline: "none", lineHeight: 1.5, opacity: loading ? 0.5 : 1 }}
        ...
        <button onClick={() => send()} disabled={loading || !input.trim()}
          style={{ background: loading || !input.trim() ? "rgba(245,158,11,0.15)" : "#f59e0b", border: "none", borderRadius: 8, padding: "9px 16px", color: loading || !input.trim() ? "#78350f" : "#0a0a0f", fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", letterSpacing: "0.06em", cursor: loading || !input.trim() ? "not-allowed" : "pointer", fontWeight: 600, whiteSpace: "nowrap" }}>
          {loading ? "THINKING..." : "▶ SEND"}
        </button>
      </div>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.3}}@keyframes buddyPulse{0%,100%{opacity:0.2}50%{opacity:0.8}}`}</style>
    </div>
  );
}
